<?php 
	...
	$attached_query = 'SELECT * FROM `'.$wpdb->prefix.'posts` where post_type="article" and ID='.$attach_id.'';
	 ...
	 $date=date_create($get_attacment[0]->post_modified);
 	 ...
 	 $user_ = get_userdata( $get_attacment[0]->post_author );
 	   ...
 	   $image_ids = get_post_meta($get_attacment[0]->ID,'upload_image',true);
...
<div class="library-preview-bottom-panel" data="<?php echo $get_attacment[0]->ID?>">
	<div class="library-asset-top-panel">
		<p class="ndl-Text ndl-Text--secondary text--medium library-asset-type-info">Original Article</p>
	</div>
	...
	<div class="library-asset-body-panel">
		...
		<?php  
			echo wp_get_attachment_image($image_ids[0],'full');
		...
		<div class="media-description truncated">
		 ...
		 <div id="accordion">
			  ...
			  <div class="labels">
			  	...
			  	<?php  if(!empty($Target_Audience2)){ ?>
					<div class="content_format ndl-Labels-container">
					   <span class="ndl-Labels-type">TARGET AUDIENCE</span>
					   <span class="ndl-Labels-item ndl-Labels-itemLast">
					   	  <?php 
						   	  	foreach($Target_Audience2 as $Target_Audience2) {
						   	  		?>
						   	  			<div class="ndl-Pill ndl-Pill--medium ">
									   		<span class="ndl-Pill-title "><?php echo $Target_Audience2;?></span>
									   </div>
						   	  		<?php 
						   	  	}
					   	  ?>
					   ...
					   <span class="ndl-Labels-item ndl-Labels-itemLast">
					   	  <?php 
						   	  	foreach($content_pillar as $content_pillars) {
						   	  		?>
						   	  			<div class="ndl-Pill ndl-Pill--medium ">
									   		<span class="ndl-Pill-title "><?php echo $content_pillars;?></span>
									   </div>
						   	  		<?php 
						   	  	}
					   	  ?>
...
.media-description .size-full {
    width: 100%;
    height: auto;
}
.ndl-InlineEditDisplayMode-wrapper {display: flex; position:relative;}
.edit-event {
    display: none;
}
...
input#article_title {
    width: 100%;
    border: none;
    border-bottom: solid 1px #807379;
    border-radius: 0;
}
</style>
<script>
